...
| 3 | data and Python environment were supplied locally | correct the sibling data root and avoid an unnecessary transformers dependency for precomputed teacher features | point configs/default loader at `code/data`; reuse stored teacher features when text is not masked; run CPU smoke | B0/B1/B2 one-epoch smoke, B1 evaluation, and robustness all completed | keep; full GPU runs remain |
