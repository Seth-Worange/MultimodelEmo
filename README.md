...
本地研发分支还提供冻结的 FUSE missing-modality 对照协议：
`config/q2_fuse_b0.yaml`（B0，无归一化、无结构化缺失）、`config/q2_fuse_b1.yaml`
（B0 加入仅用训练有效位置拟合的 audio/vision 归一化）和
`config/q2_fuse_b2.yaml`（B1 加入 local/interval 为主、低比例 whole 的结构化缺失）。
归一化统计量会写入 `best.pt`，验证、鲁棒性和专项推理自动复用；缺失与 padding 位置在
归一化后仍保持精确零值。协议、审计与实验记录位于 `outputs/q2_fuse_missing/`。
