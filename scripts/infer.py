...
from utils.normalization import (InputNormalization, apply_input_normalization,
...
class ModelEnsemble(torch.nn.Module):
    def __init__(self, models: list[AffectiveModel]):
        ...
        stats = [getattr(model, "input_normalization", None) for model in models]
        if any(normalization_summary(item) != normalization_summary(stats[0]) for item in stats[1:]):
            raise ValueError("ensemble checkpoints use different input normalization statistics")
        self.input_normalization = stats[0]
...
def load_model(path: Path | list[Path], device: torch.device) -> torch.nn.Module:
    ...
    for checkpoint_path in paths:
        ...
        model.input_normalization = InputNormalization.from_dict(checkpoint.get("input_normalization"))
        models.append(model)
...
def main() -> None:
    ...
    for path in files:
        ...
        if getattr(model, "input_normalization", None) is not None:
            item = apply_input_normalization(item, model.input_normalization)
